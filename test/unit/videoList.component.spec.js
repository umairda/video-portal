(function() {
	...
	describe('videoList component', function() {		
		...
		beforeEach(inject(function($compile,_$q_,$rootScope) {
			...
			element = angular.element('<video-list skip="skip" limit="limit" column-class="{{columnClass}}"></video-list>');
			$scope.skip = 1;
			$scope.limit = 5;
			$scope.columnClass = 'col-md-4';
		...
		it("should set the skip binding",function() {
			expect(videoListCtrl.skip).toBe($scope.skip);
		});
		
		it("should set the limit binding",function() {
			expect(videoListCtrl.limit).toBe($scope.limit);
		});
		
		it("should set the columnClass binding",function() {
			expect(videoListCtrl.columnClass).toBe($scope.columnClass);
			var findColumnClass = element[0].querySelector('div.test-column-class').getAttribute('class');
			expect(findColumnClass.search($scope.columnClass)).not.toBe(-1);
		});
